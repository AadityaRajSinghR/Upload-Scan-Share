...
async function uploadLargeFile() {
  const options = {
    ...
    headers: {
      ...
      'x-apikey': '8e4284f4b15a6c38f3f6c0b8f117a7be99f6434ccdd3b7bca54f705a785c7103',
    },
  };

  try {
    const res = await fetch('https://www.virustotal.com/api/v3/files/upload_url', options);
    ...
    uploadLargeFile()
      ...
      .then((response) => {
        if (response?.data?.links?.self) {
          ...
          setTimeout(() => {
            getFileDetails(response.data.links.self)
              .then((details) => {
